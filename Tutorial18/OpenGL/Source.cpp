...
Camera camera(camera_settings, glm::vec3(0.0, 1.0, 12.0));
...
int main()
{
	...
	GLFWwindow* window = glfwCreateWindow(camera_settings.screenWidth, camera_settings.screenHeight, "Computer Graphics: Tutorial 18", NULL, NULL);
	if (window == NULL)
	{
	...
	//Load models
	Model lowPolySphere = Model("Resources\\Models\\Sphere_low_poly.obj");

	//Load texture
	GLuint texture = TextureLoader::loadTexture("Resources\\Models\\bumblebee.png");

	//Attach texture to the model
	lowPolySphere.attachTexture(texture);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glUseProgram(perPixel);

		glUniformMatrix4fv(glGetUniformLocation(perPixel, "view"), 1, GL_FALSE, glm::value_ptr(view));
		glUniformMatrix4fv(glGetUniformLocation(perPixel, "projection"), 1, GL_FALSE, glm::value_ptr(projection));
		glUniformMatrix4fv(glGetUniformLocation(perPixel, "model"), 1, GL_FALSE, glm::value_ptr(model));

		GLint lightDirectionLoc = glGetUniformLocation(perPixel, "lightDirection");
		GLint lightDiffuseLoc = glGetUniformLocation(perPixel, "lightDiffuseColour");
		glUniform4f(lightDirectionLoc, 1.0f, 1.0f, 0.5f, 0.0f);
		glUniform4f(lightDiffuseLoc, 1.0f, 1.0f, 1.0f, 1.0f);

		lowPolySphere.draw(perPixel); //Render the model
