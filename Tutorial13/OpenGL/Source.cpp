...
Camera camera(camera_settings, glm::vec3(0.0f, 2.5f, 10.0f));
...
int main()
{
	...
	GLFWwindow* window = glfwCreateWindow(camera_settings.screenWidth, camera_settings.screenHeight, "Computer Graphics: Tutorial 13", NULL, NULL);
	if (window == NULL)
	{
	...
	//Load models
	Model boxModel = Model("Resources\\Models\\Quidditch_Player.fbx");

	//Load texture
	GLuint texture;
	texture = TextureLoader::loadTexture("Resources\\Models\\bumblebee.png");

	//Attach texture to the model
	boxModel.attachTexture(texture);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		boxModel.draw(basicShader); //Render the model
